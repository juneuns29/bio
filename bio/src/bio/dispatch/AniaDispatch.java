 ...
 */
@WebServlet("*.ania")
public class AniaDispatch extends HttpServlet {
	 ...
	 */
	public HashMap<String, AniaInter> map;
...
//			System.out.println("####### path : " + path);
			// 스트림 만들고
			fin = new FileInputStream(path + "../resources/ania.properties");
			// prop에 파일의 데이터 기억
			prop.load(fin);
...
//		System.out.println("***** login : " + prop.getProperty("/member/login.ania"));
		
		// map에 요청키값과 실행 클래스를 기억시킨다.
		map = new HashMap<String, AniaInter>();
		// 키값만 모두 꺼내온다.
		...
		while(en.hasMoreElements()) {
			...
			try {
				...
				// 인스턴스 만들고
				/*
				 	AniaInter ai = new AniaMain();
				 	AniaInter ai2 = new Login();
				 */
				AniaInter val = (AniaInter) tmp.newInstance();
